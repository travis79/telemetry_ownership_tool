...
def metric_has_mirror_heuristic(metric_obj: dict, phrases: List[str]) -> bool:
    ...
    targets = set(p.lower() for p in (phrases or [])) | base_phrases
...
def rows_from_app(app_key: str, index_obj: dict, cfg: dict, exclude_core=True, ios_codeowners: bool=False) -> List[dict]:
...
def write_markdown_summary(summary_df: pd.DataFrame, detail_df: pd.DataFrame, outdir: Path, title="Telemetry Ownership — Report"):
    ...
    for app in sorted(summary_df['application'].unique()):
        ...
        top = sub.sort_values(['total','glean_metric_count','owned_pings_count'], ascending=False).head(12)
        lines.append(f"## {app}\n| owner | owner_type | metrics | mirrored | pings |")
        lines.append("|---|---:|---:|---:|---:|")
        for _, r in top.iterrows():
            lines.append(f"| {r['owner']} | {r['owner_type']} | {int(r['glean_metric_count'])} | {int(r['mirrored_metric_count'])} | {int(r['owned_pings_count'])} |")
        lines.append("")
...
def main():
    ap = argparse.ArgumentParser(description="Telemetry ownership summarizer (Desktop default via Glean Dictionary; mobile optional; local overrides via --inputs)")
    ap.add_argument("--inputs", default="", help="Optional folder with local *index.json overrides (e.g., fenix_index.json). If absent, fetch from Glean Dictionary.")
    ap.add_argument("--outdir", required=True, help="Output folder for CSVs/Markdown")
    ap.add_argument("--apps", default="", help="Comma-separated mobile apps (e.g. fenix,focus_android,firefox_ios,focus_ios)")
    ap.add_argument("--desktop", default="firefox_desktop_index.json", help="Desktop index filename to look for locally (default). If not found, we fetch from Glean Dictionary.")
    ap.add_argument("--mirrors", default="heuristic", choices=["none","heuristic","strict"], help="Mirror mode for Desktop (default=heuristic)")
    ...
    if dindex:
        drows = rows_from_app("firefox_desktop", dindex, cfg, exclude_core=False, ios_codeowners=False)
        drows = add_desktop_mirrors(drows, dindex, args.mirrors, cfg)
        all_rows.extend(drows)
    else:
        print("[warn] Could not load Firefox Desktop index.json (local or remote).", file=sys.stderr)
    ...
    write_markdown_summary(summary, detail, outdir, title="Telemetry Ownership — Desktop (default) + Mobile (optional)")

    # Console summary
    print(f"Wrote:\n  {summary_path}\n  {detail_path}\n  {outdir/'SUMMARY.md'}")
    print(f"[apps]={summary['application'].nunique()} "
          f"[owners]={summary[['owner','application']].drop_duplicates().shape[0]} "
          f"[rows]={len(detail)}")
    d_mir = int(detail[(detail['application']=="firefox_desktop") & (detail['type']=="metric")]["mirrored"].sum())
